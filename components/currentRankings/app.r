...
###
# NEW RANKING TABLE -------------------------------------------------------
current_rankings <- read.csv("../../data/current_rankings.csv")
...
gen_ranking_table <- function(updateProgress = NULL){
  ...
  tbl <- reactable(
    ...
    columns = list(
      ...
      career_avg = ranking_column(header = (span("Raw Average (2 Season)", title = "Averge ranking points per game BEFORE old game decay and participation weighting")), format = colFormat(digits=1),
                                  cell = function(value) {
