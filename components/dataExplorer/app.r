                ...
                sidebarPanel(id='sidebar-div',
                             fluidRow(
                               column(2,selectInput("seasonInput","Season",
                                                    choices = c("All","Last5",29:14),selected = 29)),
...
server <- function(input,output,session) {
    ...
    if(input$seasonInput=="All"){
      ...
      if(input$xstat==input$ystat) {
        ...
        p<-ggplot(filtered, aes(x=reorder(name,get(input$xstat)), y=get(input$xstat), fill=factor(filtered$season))) + geom_bar(stat="identity") + coord_flip() +
          labs(title=toupper(paste("ozfortress premier",input$classInput,"seasons 14-29",sep=" ")), y=toString(labels[which(labels$lookup==input$xstat),]$string), x="", fill="Season")
        ...
        p<-ggplot(filtered, aes(x=get(input$xstat), y=get(input$ystat)
          ...
          labs(title=toupper(paste("ozfortress premier",input$classInput,"seasons 14-29",sep=" ")), x=toString(labels[which(labels$lookup==input$xstat),]$string), y=toString(labels[which(labels$lookup==input$ystat),]$string), colour="season") + theme_minimal()
  ...
  observeEvent(input$resetbutton, {
    updateSelectInput(session, "seasonInput", selected=29)
