...
ui <- fluidPage(
  ...
  column(12,
         fluidRow(column(width=12,
                         div(class = "title", h2("Current Intermediate Player Rankings"))
                         )
                  ),
         fluidRow(column(12,
...
server <- function(input,output,session) {
  ...
  current_rankings <- read.csv("../../data/current_rankings_inter.csv")
  output$edit_date <- renderText({
    paste("Rankings as at end of Season ",currentseason, " week ", currentweek, ". Last update was at: ", toString(file.info("../../data/current_rankings_inter.csv")$mtime), sep="")
  })
